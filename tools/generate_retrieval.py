...
def gen_retrieval(save_path, dataset_name, model_name, layer, resize, knn, vis):
    ...
    for sub_category in dataset_info[0]:  # all
        ...
        train_features = torch.zeros(len(train_image_fns), *model.shapes[0][1:], device=device) # train_features 用于存放每张图像经过模型提取到的特征
...
if __name__ == "__main__":
    ...
    if args.log_path is None:
        args.log_path = f'log/retrieval/retrieval_{args.dataset_name}'
